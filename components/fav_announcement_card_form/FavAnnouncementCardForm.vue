...
const submitDisabled = computed(
  () => !formValues.type || !formValues.interlocutor || !formValues.contact
);
...
const onSubmit = async (e) => {
  ...
  await favAnnouncement.addContactsFavAnnouncement(
    props.announcementId,
    {...toRaw(formValues), date: new Date()}
  );
...
<template>
  <form
    class="space-y-4 w-full bg-white border border-gray-300 rounded-lg p-4"
    @submit="onSubmit"
  >
    <div class="flex gap-2 items-center">
      <label
        class="block flex after:content-['*'] after:ml-0.5 after:text-red-500"
      >
        Type
      </label>
      <Multiselect
    ...
    <div class="flex gap-2 items-center">
      ...
      <Input
        class="bg-white placeholder:text-gray-400"
        v-model="formValues.interlocutor"
